  ...
  <div class="return-page">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <el-button v-if="canCreate" type="primary" @click="dialogVisible = true">新建退货</el-button>
      </div>
    </div>
    ...
    >
      ...
      <el-table-column label="关联销售单" min-width="160">
        <template #default="{ row }">{{ row.saleOrder?.code || row.saleOrderId || '-' }}</template>
      </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" title="新建退货（LIFO回补）" width="560px" @close="resetForm">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        <el-form-item label="产品" prop="productId">
          <el-select v-model="form.productId" filterable :filter-method="filterProducts" placeholder="选择产品" style="width: 100%">
            <el-option v-for="p in filteredProducts" :key="p.id" :label="`${p.name} (${p.code})`" :value="p.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="数量" prop="quantity">
          <el-input-number v-model="form.quantity" :min="0.01" :precision="2" :step="1" style="width: 100%" placeholder="退货数量" />
        </el-form-item>
        <el-form-item label="销售单ID">
          <el-input v-model="form.saleOrderId" clearable placeholder="可选，关联销售单" />
        </el-form-item>
...
import type { Product, ReturnOrder } from '@/types';
...
const canCreate = computed(() => has('inventory:return') || has('inventory:*'));

const list = usePaginatedList<ReturnOrder, { page?: number; pageSize?: number; keyword?: string; productId?: string }>({
  fetchFn: (params) => inventoryApi.listReturns(params as never),
  defaultPageSize: 20,
});
...
const form = reactive<{ productId: string; quantity: number | undefined; saleOrderId: string; remark: string }>({
  productId: '',
  quantity: undefined,
  saleOrderId: '',
  remark: '',
...
async function handleCreate() {
  ...
  try {
    ...
    ElMessage.success('创建成功，已 LIFO 回补');
...
<style scoped>
.return-page { max-width: 1200px; margin: 0 auto; }
