...
{
	/// <summary>
	/// khdzd_ysmanage 的摘要说明。
	/// </summary>
	public class khdzd_ysmanage : jxc.UsrControl.UserPage//System.Web.UI.Page
	{
...
string cmd="select 客户名称,sum(出库数量 * 零售价) as 金额 from (select * from (SELECT [xsid], [销售日期],[客户名称],[店名],地区,  [cpid],  ";
...
//Textbox3.Text=cmd+") b group by 客户名称  ";
			DataSet ds = DBBase.ExecuteSql4Ds (cmd+") b group by 客户名称 order by 金额 desc  ","khdzd");
...
//			}	    
		...
		{
			u.OpenIEWindowPrint(this,"khdzd_ysmanagemx.aspx",800,600);
		}
