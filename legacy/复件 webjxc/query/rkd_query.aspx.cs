...
namespace jxc.webjxc.query
{
	...
	/// rkd_query 的摘要说明。
	/// </summary>
	public class rkd_query : System.Web.UI.Page
	{
		...
		{
			string cmd = "SELECT [rkid], [产品名称], [cpid], [仓库名称], [操作员], [入库数量], [入库单价], [剩余数量], [入库日期], [确认日期], [到货确认], [备注], [库保确认] FROM [入库单]";
			cmd+=" order by 入库日期 desc";
