...
namespace jxc.webjxc.query
{
	...
	/// dqkj_query 的摘要说明。
	/// </summary>
	public class dqkj_query : jxc.UsrControl.UserPage//System.Web.UI.Page//  
	{
		...
		{
			string cmd = "SELECT [kjid], [日期], [地区], [摘要], [借方], [贷方], [余额], [其他],[凭证号] FROM [地区会计] where 1=1 ";
			string cmd1="";
			...
			cmd = "select sum(借方)as 借方,sum(贷方)as 贷方,sum(借方)- sum(贷方)as 余额 from 地区会计 where 1=1 ";
...
//			this.Datagrid1.DataBind ();
		...
		{
				u.OpenIEWindowRight(this,"dqkj_edit.aspx",550,450);
		}
