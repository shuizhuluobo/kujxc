...
namespace jxc.webjxc.query
{
	...
	/// dqzz_query 的摘要说明。
	/// </summary>
	public class dqzz_query : jxc.UsrControl.UserPage//System.Web.UI.Page//  
	{
		...
		{
			string cmd = "SELECT [zzid], [日期], [地区], [摘要], [借方], [贷方], [余额], [其他], [分类], [单据号],[凭证号] FROM [地区总账] where 1=1 ";
			string cmd1="";
			if (this.DropDownList1.SelectedIndex!=0)
				cmd1+=" and 地区='"+this.DropDownList1.SelectedValue.ToString()+"'";
			if (this.Dropdownlist2.SelectedIndex!=0)
				cmd1+=" and 分类='"+this.Dropdownlist2.SelectedValue.ToString()+"'";
			...
			DataSet ds = DBBase.ExecuteSql4Ds (cmd+cmd1+" order by 日期 desc,凭证号 desc","dbd");
			this.Datagrid1.DataSource = ds.Tables[0].DefaultView;
			this.Datagrid1.DataBind ();
		    cmd = "select sum(借方)as 借方,sum(贷方)as 贷方,sum(借方)- sum(贷方) as 余额 from 地区总账 where 1=1 ";
...
//				Label1.Text+=" 余额:"+b.ToString("f2",System.Globalization.NumberFormatInfo.InvariantInfo);
		...
		private void Button1_Click(object sender, System.EventArgs e)
		{
			...
			u.OpenIEWindowRight(this,"dqzz_edit.aspx",550,450);
		}
