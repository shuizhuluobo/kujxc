...
{
	/// <summary>
	/// rySelect_Search 的摘要说明。
	/// </summary>
	public class rySelect_Search : System.Web.UI.Page
	{
		...
		{
			string cmd = "select * from 操作员信息 where 1=1 ";
			if (this.cpname.Text != string.Empty)
				cmd += " and 操作员名称 like '%" + this.cpname.Text.Trim () + "%'";

			DataSet ds = DBBase.ExecuteSql4Ds (cmd+" order by 所在部门 desc","khSelect");
...
//			JSUtil.CloseWindow(this);
		...
		private void Datagrid1_SelectedIndexChanged(object sender, System.EventArgs e)
		{
          //  String xx=Datagrid1.SelectedItem.Cells[1].Text;
			string Selection="";
			string id = Datagrid1.SelectedItem.Cells[2].Text;
			if (id!=null)
			{
				string cmd = "select * from 操作员信息 where 1=1 and 操作员名称='"+id+"'";
				SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
				if (dr.Read ())
				{
					Selection=dr["操作员名称"].ToString()+","+dr["所在部门"].ToString();
				}
				Session.Add("Ret_Search_Value",Selection);
				JSUtil.ExecuteBlock(this,"window.returnValue=\"SubmitForm\"");
			}
