...
{
	/// <summary>
	/// gysSelect_Search 的摘要说明。
	/// </summary>
	public class gysSelect_Search : System.Web.UI.Page
	{
		...
		private void BindData ()
		{
			...
			if (this.cpname.Text != string.Empty)
				cmd += " and 供应商名称 like '%" + this.cpname.Text.Trim () + "%' or 助记码 like '%"+this.cpname.Text.Trim ()+"%'";

			DataSet ds = DBBase.ExecuteSql4Ds (cmd,"gysSelect");
		...
		private void change_Click(object sender, System.EventArgs e)
		{
			...
			if (id!=null)
			{
				string cmd = "select * from 基础信息_供应商资料 where 1=1 and 供应商ID='"+id+"'";
				SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
				if (dr.Read ())
				{
					Selection=dr["供应商ID"].ToString()+","+dr["供应商名称"].ToString()+","+dr["联系人"].ToString();
				}
