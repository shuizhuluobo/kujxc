...
namespace jxc.admin.bases
{
	...
	/// ckshmx_manage 的摘要说明。
	/// </summary>
	public class ckshmx_manage :jxc.UsrControl.UserPage//System.Web.UI.Page//  
	{
...
add.Attributes.Add("onclick","return confirm('您真的确认已收回单据？')");
		...
		{
			string cmd = "select * from (SELECT a.[xsid], a.[产品名称], a.[cpid], a.[产品型号], a.[销售数量], a.[制作明细],a.[已调拨],a.[到货确认],b.[销售日期],a.[rkid],a.[单价],a.[是否审核],a.[产品类别],b.客户名称,b.店名,b.经办人,b.地区,b.是否回单,b.审核通过,b.操作员,b.备注1 FROM [销售单明细] as a,[销售单] as b where a.xsid=b.xsid and b.标志='正常' ) as xx where 1=1 ";
			if (this.cpname.Text != string.Empty)
             ...
             if (DropDownListlx.SelectedIndex!=0)
				...
				cmd += " and 产品名称 like '%" + this.Textbox1.Text.Trim () + "%'";
			if (this.Textbox2.Text != string.Empty)
				cmd += " and xsid like '%" + this.Textbox2.Text.Trim () + "%'";
//			if (this.groupname.ToString()!="0")
//			{
...
//			u.OpenIEWindowRight(this,"xsdb_edit.aspx?id="+id,750,550);
			string id = utils.FindFirstCheckedItem(this.Datagrid1);
			u.OpenIEWindowRight(this,"cksh_bz.aspx?rkid="+id,750,550);
			BindData ();
//					    string id = utils.FindFirstCheckedItem(this.Datagrid1);
...
//						BindData ();
		...
		{
//		string id = utils.FindFirstCheckedItem(this.Datagrid1);
////			u.OpenIEWindowRight(this,"ckshmx_edit.aspx?cpid=" + id,500,500);
...
//			BindData ();
		...
		{
			this.Response.Redirect("cksh_manage.aspx",true);
