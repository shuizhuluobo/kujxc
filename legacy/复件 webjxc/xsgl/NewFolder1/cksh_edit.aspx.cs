...
//				this.Textbox3.Text=rkrq.Text;
//			    this.Textbox4.Text=rkrq.Text;
		...
		private void save_Click(object sender, System.EventArgs e)
		{
			...
			string cmd="SELECT 销售单.地区, 销售单明细.产品名称, 销售单明细.销售数量,销售单.销售日期 AS 日期, 销售单明细.xsid, 销售单.店名,销售单明细.xsdmxid, 销售单明细.cpid, 销售单明细.单价,销售单明细.零售价 FROM 销售单明细 INNER JOIN 销售单 ON 销售单明细.xsid = 销售单.xsid and ";
				...
				while (dr.Read ())
				{
					...
					cmd="select cpid,sum(剩余数量) as 总数量 from 入库单 where 剩余数量>0 and 店名='"+dr["店名"].ToString()+"' and cpid='"+dr["cpid"].ToString()+"' group by cpid ";
					dr1.Close();
					sqlCmd1.CommandText=cmd;
					dr1 = sqlCmd1.ExecuteReader();
					dr1.Read();
					if (i>Convert.ToDouble(dr1["总数量"].ToString ()))
					{
						string s=dr["产品名称"].ToString();
						...
						sqlCon1.Close();
						sqlCon2.Close();
						utils.Alert (this,s+"的数量不足，请检查库存情况，审核不通过!");
						return;
					}
					////产品数量小于库存继续，按照批次减掉产品数量
					dr1.Close();
					cmd="select * from 入库单 where 剩余数量>0 and 店名='"+dr["店名"].ToString()+"' and cpid='"+dr["cpid"].ToString()+"' order by 入库日期";
					sqlCmd1.CommandText=cmd;
					dr1 = sqlCmd1.ExecuteReader();
					while (dr1.Read () || (i!=0))
					{
						if (i>Convert.ToDouble(dr1["剩余数量"].ToString()))
						{
							cmd="INSERT INTO [销售明细批次]([xsdmxid], [cpid], [出库数量], [零售价], [rkid], [进货价], [出库日期], [可退数量])";
							cmd+="VALUES('"+dr["xsdmxid"].ToString()+"','"+dr1["cpid"].ToString()+"',";
							cmd+=dr1["剩余数量"].ToString()+","+dr["零售价"].ToString()+",'"+dr1["rkid"].ToString()+"',";
							cmd+=dr1["进货价"].ToString()+",'"+dqrq1.Trim()+"',"+dr1["剩余数量"].ToString()+")";
							sqlCmd2.CommandText=cmd;

							sqlCmd2.ExecuteNonQuery();
							cmd="update 入库单 set 剩余数量=0 where rkid='"+dr1["rkid"].ToString()+"'";
							i=i-Convert.ToDouble(dr1["剩余数量"].ToString());
							sqlCmd2.CommandText=cmd;
							sqlCmd2.ExecuteNonQuery();
						}
						else
						{
							...
							cmd+=dr1["进货价"].ToString()+",'"+dqrq1.Trim()+"',"+i.ToString()+")";
							sqlCmd2.CommandText=cmd;
							sqlCmd2.ExecuteNonQuery();
				...
				cmda[3]="update 销售单 set 审核通过='是' where xsid='"+ Textbox2.Text+"'";//update by 2010-01-29
