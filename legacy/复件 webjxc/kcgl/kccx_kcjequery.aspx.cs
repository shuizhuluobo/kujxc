...
{
	/// <summary>
	/// kccx_kcjequery 的摘要说明。
	/// </summary>
	public class kccx_kcjequery : System.Web.UI.Page
	{
...
//			DgNavigation1.SetStyle(20, false);//10表示每页分10行，true表示无分页时自动隐藏
			if (!this.Page.IsPostBack)
			{
                ...
                utils.BindDropDownList("select ListName,ListName from rs_CorSub where SortId=7",this.Dropdownlist3);
		...
		{
			string cmd = "SELECT [产品名称], [仓库名称],店名,产品类别,cpid, sum(剩余数量-供退+客退) as 库存数量,sum((剩余数量-供退+客退)*进货价) as 金额 FROM [入库单] where 单据标志='正常' and 1=1 ";
			...
			if (Dropdownlist3.SelectedIndex!=0)
				cmd+=" and 产品类别 like '%"+Dropdownlist3.SelectedValue+"%'";
			cmd+=" group by 仓库名称,店名,产品类别,cpid,[产品名称] order by 产品类别,产品名称,库存数量 ";
...
　　          // e.Item.Cells[6].Text = string.Format("{0:F2}", Convert.ToDouble(e.Item.Cells[6].Text));
				...
				runningTotal1=runningTotal1+Convert.ToDouble(e.Item.Cells[7].Text);
...
　　           e.Item.Cells[6].Text = string.Format("{0:F2}", runningTotal);
				e.Item.Cells[7].Text = string.Format("{0:F2}", runningTotal1);
//
