...
namespace jxc.webjxc.query
{
	...
	/// dkccx_query 的摘要说明。
	/// </summary>
	public class dkccx_query :jxc.UsrControl.UserPage//System.Web.UI.Page// 
	{
		...
		{
			string cmd = "select * from (SELECT [产品名称], [仓库名称],cpid, sum(剩余数量+客退-供退) as 库存数量,店名 FROM [入库单] where 1=1 ";
			string cmd1="";
			...
			cmd+=" group by 仓库名称,店名,cpid,[产品名称]) as xx where 1=1 "+cmd1+"  order by 库存数量 ";
			DataSet ds = DBBase.ExecuteSql4Ds (cmd,"dkccx");
