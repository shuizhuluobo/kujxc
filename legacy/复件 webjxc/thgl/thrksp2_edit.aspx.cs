...
namespace jxc.admin.bases
{
	...
	/// cksh_add 的摘要说明。
	/// </summary>
	public class thrksp2_edit :jxc.UsrControl.UserPage
	{
			...
			if (!this.Page.IsPostBack)
			{
				...
				else
				{
					...
					if (dr1.Read ())
					{
					    ...
					    TextBox11.Text=dr1["地区"].ToString();
					}
					dr1.Close();

				}
				Button3.Attributes.Add("onclick","return confirm('您真的确认同意退货？ 确认后该单据将完结并自动生成销售单平库!')");
		...
		private void save_Click(object sender, System.EventArgs e)
		{
			...
			string cmd="update 退货单 set 单据状态='回退',状态=2 where thid='"+id+"'";
...
//			u.OpenIEWindowRight(this,"thrksp2_addmx.aspx?thid="+Textbox2.Text,600,350);
		}
...
//			BindData ();
		...
		private void Button3_Click(object sender, System.EventArgs e)
		{
			...
			string xsid = utils.Getbm("xsid","销售单",this.glydh.ToString()+string.Format("{0:yyyyMM}",DateTime.Now),4);
			string id = this.Request.QueryString["thid"];
			string strcmd="";
			if (id!="")
			{
				strcmd="insert into 销售单 ([xsid],[店名], [总计金额], [预付定金], [客户名称], [销售日期], [取货日期], [客户电话], [备注], [经办人], [电话], [审核通过], [地区]) values('";
				strcmd+=xsid+"','"+this.rkrq.Text.ToString()+"',0-";
				strcmd+=this.Textbox8.Text.Trim()+",";
				strcmd+= "0,'";//Textbox3
				strcmd+= this.Textbox1.Text.ToString()+"','";
				strcmd+=DateTime.Now.ToString()+"','";
				strcmd+=dqrq1.ToString()+"','";
				strcmd+=this.Textbox5.Text.ToString()+"','";
				strcmd+=this.Textbox6.Text.ToString()+"','";
				strcmd+=this.czy.Text.ToString()+"','";
				strcmd+=this.Textbox7.Text.ToString()+"','";
				strcmd+="是','";
				strcmd+=this.TextBox11.Text.ToString()+"')";
			...
			try
			{
				...
				string cmd2 = "select [thmxid], [thid], [产品名称], [cpid], [产品型号], [退货数量],[rkid],[单价]*[退货数量] as chengben from 退货单明细 where thid='" + Textbox2.Text + "'";
				double chengben=0;
...
//					else
				...
				string[] cmd=new string[4];
				 id = utils.Getbm("cwid","地区财务",string.Format("{0:yyyyMMdd}",DateTime.Now),4);
				cmd[0]="insert into 地区财务( [cwid], [店名], [地区], [xsid], [客户], [经办人], [时间1], [时间2], [总金额], 预收定金, [销售成本], [其他], [日期1], [日期2], [是否结算])values('";
				cmd[0]+=id+"','";
				cmd[0]+=this.rkrq.Text.ToString()+"','";
				cmd[0]+=this.TextBox11.Text.ToString()+"','";
				cmd[0]+=Textbox2.Text+"','";
				cmd[0]+=Textbox1.Text+"','";
				cmd[0]+=this.czy.Text.ToString()+"','";
				cmd[0]+=Textbox3.Text+"','";
				cmd[0]+=Textbox3.Text+"',0-";//
				cmd[0]+=this.Textbox8.Text.Trim()+",0-";
				cmd[0]+=this.Textbox8.Text+",0-";
				cmd[0]+=chengben.ToString()+",'";
				cmd[0]+="销售收入"+"','";
				cmd[0]+=dqrq1.ToString()+"','";
				cmd[0]+=dqrq2.ToString()+"','是')";
				...
				cmd[1]+=this.TextBox11.Text.ToString()+"','";
				cmd[1]+="销售产品预收款"+"',0-";
				cmd[1]+=this.Textbox8.Text+",";
				cmd[1]+="0,0-";
				cmd[1]+=Textbox8.Text+",'";
				cmd[1]+="销售店"+this.TextBox11.Text.ToString()+"销售单号:"+Textbox2.Text+"')";
				...
				cmd[2]+=this.TextBox11.Text.ToString()+"','";
				cmd[2]+="销售总计金额"+"',0-";
				cmd[2]+=this.Textbox8.Text+",";
				cmd[2]+="0,0-";
				cmd[2]+=Textbox8.Text+",'";
				cmd[2]+="销售店"+this.TextBox11.Text.ToString()+"销售单号:"+Textbox2.Text+"')";

				//cmd[3]="update 销售单 set 单据状态='完成' where xsid='"+xsid+"'";
				cmd[3]="update 退货单 set 单据状态='完结',状态=4,总会计审核='是' where thid='"+this.Request.QueryString["thid"].ToString()+"'";
				DBBase.ExecuteSqls (cmd);
				
				utils.Alert (this,"保存并生成单据成功!");
				JSUtil.Close(this);
			}
			catch
			{
