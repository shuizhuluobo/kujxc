...
namespace jxc.webjxc.xsgl
{
	...
	/// xsprint 的摘要说明。
	/// </summary>
	public class xsprint : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack)
			{
				...
				{
					string cmd = "SELECT [xsid], [销售单号], [店名], [总计金额], [预付定金], [客户名称], [销售日期], [取货日期], [客户电话], [备注], [经办人], [电话], [审核通过], [地区] FROM [销售单] where xsid='" + id + "'";
					...
					//	dr["销售日期"].ToString ();
						...
						this.Label10.Text = dr["电话"].ToString (); 
					}
					dr.Close ();
		...
		{
			string cmd = "select * from 销售单明细 where 1=1 and xsid='"+this.Request.QueryString["id"].ToString()+"'";
...
//			{
//				//  取得 销售数量 字段的值
//				float   b = (float)DataBinder.Eval(e.Item.DataItem, "销售数量");
////				float   b=float.Parse(dr["预付定金"].ToString ());   
//				e.Item.Cells[2].Text=b.ToString("f2",System.Globalization.NumberFormatInfo.InvariantInfo);   
//				
//			}
