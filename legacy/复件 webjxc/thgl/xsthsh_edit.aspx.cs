...
namespace jxc.admin.bases
{
	...
	/// xsthsh_add 的摘要说明。
	/// </summary>
	public class xsthsh_edit :jxc.UsrControl.UserPage// System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				if (dr.Read ())
				{
					...
					Textbox11.Text=this.glyname.ToString();
				}
		...
		{    
			...
			this.PreRender += new System.EventHandler(this.xsthsh_edit_PreRender);
		...
		private void BindData ()
		{
			...
			DataSet ds = DBBase.ExecuteSql4Ds (cmd,"xsthsh");
			this.Datagrid1.DataSource = ds.Tables["xsthsh"].DefaultView;
				...
				while (dr.Read ())
				{
					...
					while (dr1.Read () || (i!=0))
					{
						...
						if (i>Convert.ToDouble(dr1["剩余数量1"].ToString()))
						{
							...
							cmd="update 入库单 set 剩余数量=剩余数量-"+dr1["剩余数量1"].ToString()+" where rkid='"+dr1["rkid"].ToString()+"'";
							//string 	str="update 入库单 set 供退=供退+"+thsl.ToString()+" where rkid ='"+dr1["rkid"].ToString()+"'";
...
//				Transaction3.Commit();
				...
				sqlCon1.Close();
				sqlCon2.Close();
				//u.OpenIEWindowRight(this,"xsthsh_zzxp.aspx?xsid="+Textbox2.Text,600,350);
				JSUtil.Close(this);
			}
			catch
			{
...
//			if (id!=null)
//			{
//				u.OpenIEWindowPrint(this,"xsthsh_zzxp.aspx?xsdmxid="+id,750,550);
...
//			}
		...
		private void CodeSearch()
		{
			...
			if(!Page.IsPostBack)
			{
				...
				strScript = JSUtil.GetOpenDialogScript("人员选择","../CommonSearch/rySelect.aspx",550,650,"xsthsh");
			...
			JSUtil.ExecuteBlock(this,"parent.frames[\"xsthsh\"].xsthsh.HiddenCommon.value=\"\"");
		...
		private void xsthsh_edit_PreRender(object sender, System.EventArgs e)
		{
