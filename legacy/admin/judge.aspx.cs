...
namespace jxc.admin
{
	...
	/// judge 的摘要说明。
	/// </summary>
	public class judge : jxc.UsrControl.UserPage
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if (!this.Page.IsPostBack)
			{
				...
				{
					if (this.hidtype.Text == "0")
					{
						cmd = "select 1 from cnc_stream where judger='" + this.judger + "' and type=0 and sortid=(select sortid from cnc_stream where judger='" + this.judgeczy.Text + "' and type=" + this.hidtype.Text + ")+1";
						dr = DBBase.ExecuteSqlReader (cmd);
						if (!dr.HasRows)
						{
							this.savesp.Enabled = false;
							this.report.Text = "你对该环节没有审批权利";
						}
						dr.Close ();
					}
				}
		...
		private void savesp_Click(object sender, System.EventArgs e)
		{
			...
			string iffinish;
			string sortid1="1",sortid2="2";

			if (this.cptype.Text == "0")
			{
				if (this.hidtype.Text == "0")
				{
					if (this.judger == this.hidfinishczy.Text.Trim ())
					{
						iffinish = "1";
					}
					else
				...
				{
					string cmd = "select glydh,result from t_master_child where bh=" + this.Request.QueryString["id"];
					...
					else //其他三人都审批完成
					{
						...
						{
							if (ds.Tables[0].Rows[i]["result"].ToString () == "不同意")
							{
								allpass = false;
								break;
							}
						}
						if (allpass == false)   //有不同意见
						{
							bool allnotpass = true;
							for (int i=0;i<ds.Tables[0].Rows.Count;i++)  //判断是否是全部不同意
							{
								if (ds.Tables[0].Rows[i]["result"].ToString () == "同意")
								{
									allnotpass = false;
									break;
								}
							}
							if (allnotpass == true)   //全部都不同意
							{
								if (this.RadioButtonList1.SelectedIndex == 0)
								{
									iffinish = "2";   //我同意 等批
