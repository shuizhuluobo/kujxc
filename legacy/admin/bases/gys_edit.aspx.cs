...
namespace jxc.admin.bases
{
	...
	/// gys_add 的摘要说明。
	/// </summary>
	public class gys_edit : jxc.UsrControl.UserPage//System.Web.UI.Page//
	{
		...
		protected System.Web.UI.WebControls.TextBox gysid;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			if (!this.Page.IsPostBack)
			{
				...
				string id = this.Request.QueryString["gysid"];
				if (id != string.Empty && id != null)
				{
					string cmd = "select * from 基础信息_供应商资料 where 供应商ID='" + id + "'";
					SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
					if (dr.Read ())
					{
						this.cpname.Text = dr["供应商名称"].ToString ();
						this.gysid.Text = dr["供应商ID"].ToString ();
...
//						}
		...
		private void save_Click(object sender, System.EventArgs e)
		{
			...
			if (id != string.Empty && id != null)
			{
				...
				//				this.txtyhzh.Text = dr["银行帐号"].ToString ();
				//				this.txtdq.Text = dr["地区"].ToString ();
				cmd[0] = "update 基础信息_供应商资料 set 供应商名称='" + this.cpname.Text.Trim () + "',";
			...
			{			
				string cmd="select * from 基础信息_供应商资料 where   供应商名称='"+cpname.Text.ToString()+"'";
			if (DBBase.IsValuesExists(cmd)==true)
			{
                utils.Alert (this,"该供应商名称已经存在!");
				return;
			}
				string cpid = utils.Getbm("供应商ID","基础信息_供应商资料",string.Format("{0:yyyyMM}",DateTime.Now),3);
    this.gysid.Text=cpid;
//				this.cpname.Text = dr["供应商名称"].ToString ();
//				this.gysid.Text = dr["供应商ID"].ToString ();
//				this.txtdh.Text = dr["电话"].ToString ();
//				this.txtcz.Text = dr["传真"].ToString ();
...
//				this.txtyhzh.Text = dr["银行帐号"].ToString ();
//				this.txtdq.Text = dr["地区"].ToString ();	
				cmd = "INSERT INTO [基础信息_供应商资料](供应商名称,供应商ID,电话,传真,联系人,单位地址,开户银行,银行帐号,地区,助记码) VALUES(";
				cmd += "'" + cpname.Text.Trim() + "','" + this.gysid.Text.Trim () + "','" + this.txtdh.Text.Trim () + "','" + this.txtcz.Text + "',";
