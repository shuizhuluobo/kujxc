...
{
	/// <summary>
	/// BigClass_Manage 的摘要说明。
	/// </summary>
	public class BigClass_Manage : System.Web.UI.Page
	{
		...
		private void BindData ()
		{
			...
			DataSet ds = oper.ReturnDt ("select bigid,bigname,begindate,enddate,sortid,(case ynstop when 0 then '正常' else '停用' end) as ynstop from cnc_big order by sortid asc","cnc_big");
			this.Datagrid1.DataSource = ds.Tables["cnc_big"].DefaultView;
		...
		{
			string direction = "BigClass_Add.aspx";
		...
		private void delete_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				DBBase.ExecuteSql ("delete from cnc_big where bigid=" + id);
			...
			if (this.Page.IsPostBack)
			{
				...
				foreach (DataGridItem _item in this.Datagrid1.Items) 
				{
					if (((CheckBox) _item.Cells[0].FindControl("selectcheck")).Checked)
					{
						...
						{
							string updatecmd = "update cnc_big set sortid = " +  ((TextBox)_item.Cells[6].FindControl("sortid")).Text.Trim() + "where bigid =" +   Datagrid1.DataKeys [_item.ItemIndex].ToString().Trim ();
							oper.Exec (updatecmd);
						}
					}
				}
		...
		private void change_Click(object sender, System.EventArgs e)
		{
			...
			u.OpenIEWindowRight(this,"BigClass_Change.aspx?bigid=" + id,600,500);
		}
	}
}
