...
namespace jxc.admin.bases
{
	...
	/// dqkj_add 的摘要说明。
	/// </summary>
	public class dqkj_edit :jxc.UsrControl.UserPage//System.Web.UI.Page// 
	{
		...
		{    
			...
			this.PreRender += new System.EventHandler(this.dqkj_edit_PreRender);
		...
		{
		    string id = utils.Getbm("kjid","地区会计",string.Format("{0:yyyyMMdd}",DateTime.Now),4);
			string cmd="insert into 地区会计(kjid,日期,地区,摘要,借方,贷方,余额,其他)values('";
			cmd+=id+"','"+Textbox1.Text.ToString()+"','"+this.Textbox2.Text.ToString()+"','"+zhaiyao.Text.ToString()+"',"+Textbox4.Text.ToString();
			cmd+=","+Textbox3.Text+","+Textbox6.Text+",'"+Textbox5.Text+"')";
		...
		private void dqkj_edit_PreRender(object sender, System.EventArgs e)
		{
