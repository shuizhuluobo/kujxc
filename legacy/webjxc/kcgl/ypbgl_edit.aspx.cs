...
namespace jxc.admin.bases
{
	...
	/// spdb_add 的摘要说明。
	/// </summary>
	public class ypbgl_edit :jxc.UsrControl.UserPage//System.Web.UI.Page// 
	{
...
//				this.czy.Text=this.glyname.ToString();
				utils.BindDropDownList("select 样品类别,样品类别 from 样品类别 where 是否下柜='否'",this.DropDownList1);
...
//					dr.Close ();
		...
		{    
			...
			this.PreRender += new System.EventHandler(this.ypbgl_edit_PreRender);
		...
		private void save_Click(object sender, System.EventArgs e)
		{
			...
			{
                utils.Alert (this,"入库数量不能为0,或为负数!");
				return;
			}
			if (this.DropDownList1.SelectedIndex==0) 
			{
				utils.Alert (this,"样品类别不能空!");
				return;
			}
			if (this.DropDownList2.SelectedIndex==0) 
			{
				utils.Alert (this,"店名不能空!");
				return;
			}
            ...
            cmd="insert into 样品入库单 ([yprkid], [产品名称], [cpid], [产品型号], [入库数量], [剩余数量], [备注], [入库日期], [到货确认],[rkid],[单价],[产品类别],[店名],[仓库名称],[操作员]) values('";
			cmd+=xsdmxid+"','";
			...
			cmd+=this.Textbox6.Text.ToString()+",'";
			cmd+=this.Textbox7.Text.ToString()+"','";
			cmd+=this.Textbox3.Text.ToString()+"','";
			cmd+="否','"+this.rkid.Text+"',"+danjia.Text+",'"+this.DropDownList1.SelectedValue+"','"+this.DropDownList2.SelectedValue+"','"+this.zjgmc.ToString()+"','"+this.glyname.ToString()+"')";
			try
			{
		...
		private void CodeSearch()
		{
			...
			if(!Page.IsPostBack)
			{
				...
				strScript = JSUtil.GetOpenDialogScript("产品选择","../CommonSearch/sprk.aspx",550,500,"ypbgl_edit");
			...
			JSUtil.ExecuteBlock(this,"parent.frames[\"ypbgl_edit\"].ypbgl_edit.HiddenCommon.value=\"\"");

		}

		private void ypbgl_edit_PreRender(object sender, System.EventArgs e)
		{
