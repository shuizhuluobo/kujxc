...
namespace jxc.admin.bases
{
	...
	/// spxb_add 的摘要说明。
	/// </summary>
	public class spxb_change :jxc.UsrControl.UserPage//System.Web.UI.Page// 
	{
...
//				DBBase.ExecuteSql(cmd);
				txtgys.Text= this.Request.QueryString["gysmc"].ToString();
				Textbox4.Text = this.Request.QueryString["dhdid"].ToString();//utils.Getbm("入库单编号","下拨单",this.glydh.ToString()+string.Format("{0:yyyyMMdd}",DateTime.Now),4);
				//txtbz.Text=this.Request.QueryString["khmc"].ToString();
		...
		private void CodeSearch1()
		{
			...
			if(!Page.IsPostBack)
			{
				...
				strScript = JSUtil.GetOpenDialogScript("客户选择","../CommonSearch/khselect.aspx",380,400,"spxb_change");
			...
			JSUtil.ExecuteBlock(this,"parent.frames[\"spxb_change\"].spxb_change.HiddenCommon.value=\"\"");
		}
		...
		{    
			...
			this.PreRender += new System.EventHandler(this.spxb_change_PreRender);
		...
		private void CodeSearch()
		{
			...
			if(!Page.IsPostBack)
			{
				...
				strScript = JSUtil.GetOpenDialogScript("产品选择","../CommonSearch/spselect.aspx",550,650,"spxb_change");
		...
		private void CodeSearch2()
		{
			...
			if(!Page.IsPostBack)
			{
				...
				strScript = JSUtil.GetOpenDialogScript("供应商选择","../CommonSearch/gysSelect.aspx",380,400,"spxb_change");
		...
		private void spxb_change_PreRender(object sender, System.EventArgs e)
		{
...
　　           e.Item.Cells[6].Text = string.Format("{0:F2}", runningTotal);
		...
		private void Button3_Click(object sender, System.EventArgs e)
		{
			...
			//			u.OpenIEWindowRight(this,"spxb_change.aspx?cpid=" + id,500,500);
