...
namespace jxc.admin.bases
{
	...
	/// spdb_add 的摘要说明。
	/// </summary>
	public class xsck_addmx :jxc.UsrControl.UserPage//System.Web.UI.Page// 
	{
...
//					dr.Close ();
		...
		{    
			...
			this.PreRender += new System.EventHandler(this.xsck_addmx_PreRender);
		...
		private void save_Click(object sender, System.EventArgs e)
		{
			...
			{
                utils.Alert (this,"销售数量不能为0");
				return;
			}
			if (Convert.ToDouble(this.danjia.Text)>=Convert.ToDouble(this.Textbox3.Text))
			{
                 utils.Alert (this,"当前销售价格低于成本价！");
			}
				string cmd;
		...
		private void CodeSearch()
		{
			...
			if(!Page.IsPostBack)
			{
				...
				strScript = JSUtil.GetOpenDialogScript("产品选择","../CommonSearch/sprk.aspx",650,650,"xsck_addmx");
				...
				if (Request["HiddenCommon"]!=null && Request["HiddenCommon"]!="")
				{
					switch(Request["HiddenCommon"].ToString())
					{
						case"产品选择":
							...
							if (strs[0].ToString()!="")
							{
								string cmd = "select * from 销售单明细 where xsid='" + Textbox2.Text + "' and cpid='"+strs[1].ToString()+"' and 颜色='"+strs[6]+"'  and 规格='"+strs[8]+"'";
								SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
								if (dr.HasRows)
								{
									this.cpname.Text ="";
			...
			JSUtil.ExecuteBlock(this,"parent.frames[\"xsck_addmx\"].xsck_addmx.HiddenCommon.value=\"\"");

		}

		private void xsck_addmx_PreRender(object sender, System.EventArgs e)
		{
