...
//			string cmd="SELECT 地区,店名,cpid,rkid,产品名称,sum(销售数量) as 总数量 FROM V销售明细 where 1=1 ";
//			cmd+=" and xsid='"+this.Textbox2.Text+"' group by 地区,店名,rkid,cpid,产品名称";
...
//				cmd="select cpid,sum(剩余数量-供退+客退) as 总数量 from 入库单 where (单据标志='正常' or 单据标志='结转') and (剩余数量-供退+客退)>0 and 店名='"+dr["店名"].ToString()+"' and cpid='"+dr["cpid"].ToString()+"' and rkid='"+dr["rkid"].ToString()+"' group by cpid ";
...
//            return "0";
			...
			{
				i=Convert.ToDouble(dr["总数量"].ToString ());
				cmd="select cpid,sum(剩余数量) as 总数量 from 入库单 where (单据标志='正常' or 单据标志='结转') and 剩余数量>0 and 店名='"+dr["店名"].ToString()+"' and cpid='"+dr["cpid"].ToString()+"' and rkid='"+dr["rkid"].ToString()+"' group by cpid ";
				SqlDataReader dr1 =DBBase.ExecuteSqlReader(cmd);
				dr1.Read();
				s=dr["产品名称"].ToString();
				if (dr1.HasRows)
...
//			}
			...
			string cmd="SELECT 销售单明细.rkid,销售单.地区, 销售单明细.产品名称, 销售单明细.销售数量,销售单.销售日期 AS 日期, 销售单明细.xsid, 销售单.店名,销售单明细.xsdmxid, 销售单明细.cpid, 销售单明细.单价,销售单明细.零售价 FROM 销售单明细 INNER JOIN 销售单 ON 销售单明细.xsid = 销售单.xsid and ";
			       cmd+=" 销售单明细.xsid='"+this.Textbox2.Text+"'";
				   sqlCmd.CommandText=cmd;
				   SqlDataReader dr =sqlCmd.ExecuteReader();//销售列表
			...
			string s="";
				while (dr.Read ())
				{
					...
					////产品数量小于库存继续，按照批次减掉产品数量
						cmd="update 入库单 set 剩余数量=剩余数量-"+i.ToString()+" where rkid='"+dr["rkid"].ToString()+"'";
...
//				cmda[2]+="销售店"+this.jgmc.ToString()+"销售单号:"+Textbox2.Text+"')";
				cmda[0]="update 销售单 set 审核通过='是',操作员='"+this.Textbox11.Text+"' where xsid='"+ Textbox2.Text+"'";//update by 2010-01-29
