...
namespace jxc.admin.bases
{
	...
	/// xsckmx_manage 的摘要说明。
	/// </summary>
	public class xsckmx_manage :jxc.UsrControl.UserPage//System.Web.UI.Page//  
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack)
			{
				...
				change.Attributes.Add("onclick","return confirm('您真的确认该单据作废吗？')");
			}
		}
		...
		{
			string cmd = "select * from (SELECT a.[xsid], a.[产品名称], a.[cpid], a.[产品型号], a.[销售数量], a.[制作明细],a.[已调拨],b.[销售日期],a.[rkid],a.[单价],a.[是否审核],a.[产品类别],b.客户名称,b.店名,b.经办人,b.地区,b.到货确认,b.审核通过,b.总计金额,b.发票状态,b.收款状态,a.零售价,b.备注,b.test FROM [销售单明细] as a,[销售单] as b where a.xsid=b.xsid and b.标志='正常' ) as xx where 1=1 ";
			...
			if (CheckBox1.Checked)
				...
				cmd+=" and xsid like '%" + this.Textbox4.Text.Trim () + "%'";
			if (this.txtbz.Text != string.Empty)
				cmd+=" and 备注 like '%" + this.txtbz.Text.Trim () + "%'";
			...
			cmd+=" order by test desc ";
			DataSet ds = DBBase.ExecuteSql4Ds (cmd,"xsckmx");
			this.Datagrid1.DataSource = ds.Tables["xsckmx"].DefaultView;
		...
		private void add_Click(object sender, System.EventArgs e)
		{
			...
			u.OpenIEWindowRight(this,"xsck_edit.aspx?rkid="+id,750,550);
		...
		{
			string id = utils.FindFirstCheckedItem(this.Datagrid1);
			string cmd = "select * from 销售单 where 审核通过='否' and xsid='" +id.ToString()+ "' and 经办人='"+this.glyname.ToString()+"'";
			SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
			if (dr.HasRows==false)
			{
               dr.Close();
               id="0";
                utils.Alert (this,"审核通过的记录不能作废!");
			   return;

			}
			cmd="update 销售单 set 标志='作废' where xsid='"+id+"'";
		...
		{
			this.Response.Redirect("xsck_manage.aspx",true);
	//	u.OpenIEWindowPrint(this,"xsck_manage.aspx",750,550);
		...
		{
		string id = utils.FindFirstCheckedItem(this.Datagrid1);
			string cmd = "select * from 销售单 where xsid='"+id.ToString()+"' and 审核通过='是'";
			SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
			if (dr.HasRows)
			{
			...
			dr.Close();
			cmd = "select * from 销售单 where 经办人='"+this.glyname.ToString()+"' and xsid='"+id.ToString()+"'";
			dr = DBBase.ExecuteSqlReader (cmd);
			if (dr.HasRows)
			{
				if (dr.Read())
				{
					u.OpenIEWindowRight(this,"xsck_change.aspx?rkid="+id,750,550);
					dr.Close();
					BindData ();
					return;
				}
			...
			{
				dr.Close();
				utils.Alert (this,"当前操作员不能修改该销售单！");
				return;
			}
		...
		{
			string id = utils.FindFirstCheckedItem(this.Datagrid1);
			string cmd="update 销售单 set 收款状态='已收' where xsid='"+id+"'";
			DBBase.ExecuteSql (cmd);
			BindData ();
		}
	}
