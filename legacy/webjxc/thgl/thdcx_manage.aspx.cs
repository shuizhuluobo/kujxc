...
namespace jxc.admin.bases
{
	...
	/// thdcx_manage 的摘要说明。
	/// </summary>
	public class thdcx_manage :jxc.UsrControl.UserPage//System.Web.UI.Page//  
	{
		...
		{
			string cmd = "select * from (SELECT a.[thid], a.[产品名称], a.[cpid],a.[产品型号], a.[退货数量],b.[退货日期],a.[rkid],a.[单价],a.[是否审核],b.[单据状态],a.[产品类别],b.客户名称,b.店名,b.经办人,b.地区,b.主管审核,b.总会计审核,b.销售单号,b.状态 FROM [退货单明细] as a,[退货单] as b where a.thid=b.thid ) as xx where 1=1  ";
			...
			DataSet ds = DBBase.ExecuteSql4Ds (cmd,"thdcx");
			this.Datagrid1.DataSource = ds.Tables["thdcx"].DefaultView;
...
//				}
	...
	//	u.OpenIEWindowPrint(this,"thrk_manage.aspx",750,550);
		...
		{
			string id = utils.FindFirstCheckedItem(this.Datagrid1);
			u.OpenIEWindowRight(this,"thdcx_edit.aspx?thid="+id,750,550);
//			string id = utils.FindFirstCheckedItem(this.Datagrid1);
//			if (id!="")
//			{
