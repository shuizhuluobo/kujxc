...
namespace jxc.admin.bases
{
	...
	/// sprk_Search 的摘要说明。
	/// </summary>
	public class sprk_Search : jxc.UsrControl.UserPage//System.Web.UI.Page//
	{
		...
		private void BindData ()
		{
			...
			string cmd = "select *,进货价*1.17 as 单价,(剩余数量-供退+客退) as 实际数量 from 入库单 where (单据标志='正常' or 单据标志='结转') and (剩余数量-供退+客退)>0 and 店名='"+this.jgmc.ToString()+"'";
           ...
           if (this.DropDownList1.SelectedValue!="所有")
			...
			DataSet ds = DBBase.ExecuteSql4Ds (cmd+" order by 产品名称,入库日期 desc","sprk");
...
//			JSUtil.CloseWindow(this);
		...
		private void Datagrid1_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			if (id!=null)
			{
				string cmd = "select 入库单.*,产品信息.零售价,(剩余数量-供退+客退) as 实际数量 from 入库单,产品信息  where 入库单.cpid = 产品信息.cpid and rkid='"+id+"'";
				SqlDataReader dr = DBBase.ExecuteSqlReader (cmd);
				if (dr.Read ())
				{
					Selection=dr["产品名称"].ToString()+","+dr["cpid"].ToString()+","+dr["rkid"].ToString()+","+dr["进货价"].ToString()+","+dr["产品类别"].ToString()+","+dr["零售价"].ToString()+","+dr["颜色"].ToString()+","+dr["型号"].ToString()+","+dr["规格"].ToString()+","+dr["实际数量"].ToString();
				}
				Session.Add("Ret_Search_Value",Selection);
		    	JSUtil.ExecuteBlock(this,"window.returnValue=\"SubmitForm\"");
			}
