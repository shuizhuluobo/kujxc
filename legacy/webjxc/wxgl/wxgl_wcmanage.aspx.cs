...
namespace jxc.admin.bases
{
	...
	public class wxgl_wjmanage : jxc.UsrControl.UserPage
	{
		...
		{
			string cmd = "select wxid, 用户单位, 联系人, 联系电话, 故障信息, 备注, 接收人, 转接人, 解决问题, 登记日期, 接货时间, 维修类别, 记录状态, 货物状态, 分值, 参与人员, 记录分组,登记人 ";
            ...
            if(this.Checkbox4.Checked)
				cmd+=" and 销售标志 ='否'";
			DataSet ds = DBBase.ExecuteSql4Ds (cmd+" order by 登记日期 desc","ddxs");
		...
		private void Datagrid1_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				if (isManager == "未接收")
				{
					...
					e.Item.Cells[16].ForeColor=System.Drawing.Color.Red;
...
//				d1.DataBind();
		...
		private void Datagrid1_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{//this.glyname.ToString()
				...
				string cmd="update 维修记录 set 完结人='"+this.glyname.ToString()+"',记录状态='已完成',完结日期='"+str+"' where wxid="+id;
				DBBase.ExecuteSql (cmd);
		...
		private void Datagrid1_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{//this.glyname.ToString()
				string str=string.Format("{0:yyyy-MM-dd HH:mm:ss}",DateTime.Now);
				string cmd="update 维修记录 set 接收人='',记录状态='未接收',接货时间='' where wxid="+id;
				DBBase.ExecuteSql (cmd);
				//myClass.upDateArea(id,code,name);
				Datagrid1.EditItemIndex = -1;
				//	DG1.DataSource = myClass.bindGrid();
