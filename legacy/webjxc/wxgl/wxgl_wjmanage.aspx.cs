...
namespace jxc.admin.bases
{
	...
	/// wxgl_wjmanage 的摘要说明。
	/// </summary>
	public class wxgl_wjmanage : jxc.UsrControl.UserPage
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!this.Page.IsPostBack)
			{
				...
				cpname.Text=this.glyname.ToString();
		...
		private void BindData ()
		{
			...
			if (this.cpname.Text != string.Empty)
				cmd += " and 接收人 like '%" + this.cpname.Text.Trim () + "%'";
			...
			if (CheckBox1.Checked)
				cmd+=" and 登记日期 between '"+this.Textbox1.Text.ToString()+"' and '"+this.Textbox2.Text.ToString()+"' ";
...
//			u.OpenIEWindowRight(this,"ddxs_edit.aspx?cpid=" + id,500,500);
		...
		private void Datagrid1_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			string name2 = ((TextBox)e.Item.Cells[4].Controls[0]).Text;
          ...
          //  Dropdownlist dw1=DataGrid.FindControl("dropId") as Dropdownlist; 
			...
			{//this.glyname.ToString()
				...
				if (s=="简易")
				 {
				    cmd="update 维修记录 set 备注='"+bz+"', 完结人='"+this.glyname.ToString()+"',维修类别='"+s+"',参与人员='"+name2+"',记录状态='已完成',分值=0.5,完结日期='"+str+"' where wxid="+id;
				}
				if (s=="一般")
				{
					 cmd="update 维修记录 set 备注='"+bz+"', 完结人='"+this.glyname.ToString()+"',维修类别='"+s+"',参与人员='"+name2+"',记录状态='已完成',分值=1,完结日期='"+str+"' where wxid="+id;
				}
				if (s=="复杂")
				{
					 cmd="update 维修记录 set 备注='"+bz+"', 完结人='"+this.glyname.ToString()+"',维修类别='"+s+"',参与人员='"+name2+"',记录状态='已完成',分值=0,完结日期='"+str+"' where wxid="+id;
				}
				if (s=="待施工")
				{
					cmd="update 维修记录 set 备注='"+bz+"', 完结人='"+this.glyname.ToString()+"',维修类别='"+s+"',参与人员='"+name2+"',记录状态='已完成',分值=0,完结日期='"+str+"' where wxid="+id;
				...
				if (s=="看现场")
				{
					cmd="update 维修记录 set 备注='"+bz+"', 完结人='"+this.glyname.ToString()+"',维修类别='"+s+"',参与人员='"+name2+"',记录状态='已完成',分值=1.5,完结日期='"+str+"' where wxid="+id;
				}
				DBBase.ExecuteSql (cmd);
