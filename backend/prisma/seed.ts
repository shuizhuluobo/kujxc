...
async function main() {
    ...
    const businessRole = await prisma.role.upsert({
        ...
        create: {
            ...
            code: 'business',
            permissions: [
                'workOrder:create', 'workOrder:list', 'workOrder:view', 'workOrder:update', 'workOrder:delete', 'workOrder:export',
                'customer:list', 'customer:view', 'customer:create', 'customer:edit',
                'region:list', 'region:view',
                'serviceType:list', 'serviceType:view',
                'system:user_view',
                'wiki:list', 'wiki:view', 'wiki:create', 'wiki:edit',
                'profile:profile', 'profile:view', 'profile:edit', 'profile:change_password', 'profile:change_avatar',
            ]
        },
    });
    ...
    const engineerRole = await prisma.role.upsert({
        ...
        create: {
            ...
            code: 'engineer',
            permissions: [
                'workOrder:list', 'workOrder:view', 'workOrder:receive', 'workOrder:complete',
                'workOrder:create', 'workOrder:update', 'workOrder:delete',
                'customer:list', 'customer:view',
                'region:list', 'region:view',
                'serviceType:list', 'serviceType:view',
                'wiki:list', 'wiki:view', 'wiki:create', 'wiki:edit', 'wiki:delete',
                'system:user_view',
                'profile:profile', 'profile:view', 'profile:edit', 'profile:change_password', 'profile:change_avatar',
            ]
        },
    ...
    console.log('✅ 用户已创建: admin, business, zhangsan, lisi (密码请使用环境变量 SEED_PASSWORD 设置)');

    // 6. 创建客户
    const customersData = [
        { name: '中国银行总行', contact: '王经理', phone: '13800138000', address: '北京市复兴门内大街1号' },
        { name: '国家电网信息部', contact: '刘主任', phone: '13900139000', address: '北京市西城区西长安街86号' },
        { name: '清华大学计算机系', contact: '张老师', phone: '010-62780000', address: '北京市海淀区清华园' },
    ];

    const customerMap = new Map();
    for (const c of customersData) {
        const customer = await prisma.customer.upsert({ // 注意：这里简化逻辑，假设名字唯一，实际可能需要更复杂判断
