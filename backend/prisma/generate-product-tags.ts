...
function main() {
  return prisma.$transaction(async (tx) => {
    ...
    const tagRows = await tx.productTag.findMany({
    ...
    const relations = productTags
      ...
      .map((r) => ({
        ...
        tagId: tagIdByName.get(r.tagName) as string,
      }));
    ...
    console.log(
      `合计: ${relations.length} 条标签关联, 覆盖 ${new Set(relations.map((r) => r.productId)).size} 个产品`,
    );
  });
}
